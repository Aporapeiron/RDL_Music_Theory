...
現在の方針：ユーザー指定により声部追跡・標的検出の追加試行と楽器別比較を保留する。T03〜T06は未回答のまま保持し、正答や誤答を補完しない。既存音源・原発言・T02の課題理解訂正は保存する。Music本線は[旋律変形](../Melody_Transformation/README.md)へ進む。

[音価標的検出pilot](音価標的検出pilot.md)：Bの短音の有無を回答する別fixture。符号化試行と採点情報を分離し、人間回答を待つ。

[少数聴取小括](少数聴取小括.md)：単独認識、混合時の前景感、指定B追跡の不確かさを整理。追跡成績の確定と大人数検証は保留。

[文献参照型の観測方針](文献参照型の観測方針.md)：既存研究の課題設計を参照して進める。大人数での独自観測は保留。自由前景報告と指定声部の追跡・標的検出を区別する。
...
[指定声部B追跡の試行](指定声部B追跡_同一音源試行.md)：別旋律が前に出るとの報告あり。B追跡は「微妙」「言われればそうかも」とされ、成否・位置は未確定。
