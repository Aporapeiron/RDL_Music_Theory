...
状態：最初のdevice fixture。実聴取未記録。

## 音楽的な問い

同じ二旋律の一方だけをオクターブ移動すると、各旋律を追うことや旋律の帰属がどう変わるか。声部ID A/Bは音列の系譜として固定し、瞬間的な上声・下声とは分ける。
...
actual_listening_observationはnull。構造上の上下逆転と人間の帰属切替を分離し、実聴取からEを扱う際は、どのM_Bの予測と後続解釈を比較したかを別途明示する。
