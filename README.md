...
全体の設計方針は `03_RDL音楽_全体設計方針.md` に置く。具体的な調律・和声・旋律・リズム・楽式などは、Coreへ集積せず、必要になった時点で検証結果とともに分岐させる。`M_B`は、Bのもとで現在の解釈・予測・応答を拘束する自己側の有限整合構造として扱う。Music対象側は `SILN_music / W_music`、実入力断面は `EFP` として切り出し、`F = interp(M_B, EFP)` と分ける。ΓはCoreの状態変数ではなく、Adapter・Module・検証系に置く。
