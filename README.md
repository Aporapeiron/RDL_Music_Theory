...
50_T2_Fixtures/
    Musicの具体Probe/Selection条件と汎用検査道具候補の接続fixture

70_T2_Extraction/
    旧T2候補のT1工程・T2検査道具・Music固有基準・historicalへの再分類
...
全体の設計方針は `03_RDL音楽_全体設計方針.md` に置く。具体的な調律・和声・旋律・リズム・楽式などは、Coreへ集積せず、必要になった時点で検証結果とともに分岐させる。`M_B`は、Bのもとで現在の解釈・予測・選択・応答・更新を拘束する自己側の有限関係拘束構造として扱う。Music対象側は `SILN_music / W_music`、実入力断面は `EFP` として切り出し、`F = interp(M_B, EFP)` と分ける。ΓはCoreの状態変数ではなく、Adapter・Module・検証系に置く。`C_rel / I(M_B)` は関係拘束強度の任意補助descriptorとして必要時だけ使い、普遍音楽定数や実聴取事実とは扱わない。
...
旧 `B + M_B + Γ → C(B, M_B;Γ)` 配置を新T0/T1上で再読する、音楽側固有部分の最小検証：`10_検証/05_Bから候補空間生成_最小実験.md`

実験用スクリプト：`10_検証/boundary_candidate_generation.py`

波形関係から対象側 `SILN_music / W_music` と自己側 `M_B` 候補を分けて読む条件の最小検証：`10_検証/06_波形関係_二正弦波_最小実験.md`
