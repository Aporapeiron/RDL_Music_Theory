# 検証記録：動態Adapter候補・二標本と一標本の圧縮

*対象：24〜30で確認した音程Module・リズムModuleの動態境界*
*状態：DRAFT v0.2 / Adapter候補の証拠範囲圧縮*
*参照：`10_検証/24_音程分解_動態Adapterの最小境界_検証.md`〜`30_同一音程fallback遷移_投影と候補再構成_最小実験.md`*

---

## ■ 0. 目的と範囲

24〜30は、音程ModuleとリズムModuleを同じ内部状態や同じ因果規則へ統合するための検証ではない。異なるModule固有記録を、どの最小境界まで共通イベントとして読めるかを段階的に確かめた記録である。

29は新しい実験器・共通projector・共通状態・controllerを追加しない。既存の検証結果を、証拠が二標本にあるもの、一標本に留まるもの、まだ確認していないものへ分ける。
...
二標本で比較した接続形式
  Module固有の構造遷移record
    ...
    → recordのresulting conditionによる候補再生成
...
この区別を保つ限り、`structural_transition`は単なる分類名に留まらず、二つのModuleで後続候補生成条件へ接続された実遷移の投影として読める。一方で、その接続を共通Adapter・共通状態・共通因果構造へ一般化する根拠は、まだ存在しない。
