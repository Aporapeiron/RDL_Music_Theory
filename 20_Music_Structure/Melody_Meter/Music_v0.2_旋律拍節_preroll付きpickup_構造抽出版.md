...
downbeat_entry_control:
  meter history exists, but melody starts on downbeat
```
...
しかし、meter history alone does not create pickup:
  same pre-roll + downbeat entry
  -> arrival candidate, not pickup candidate
```

したがって、弱起は `pickup_offset` と `meter_history` の相互作用として扱う必要がある。
...
pre-roll付きpickup:
  pickup_offset と meter_history をさらに分けた
```
