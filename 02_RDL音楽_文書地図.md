...
46〜48の横断検証から、`ξ_target_candidate_generation` の現在地を抽出する。function annotation、target候補生成規則、適用条件、生成済み候補集合、selected targetを非同一として保持し、candidate setが `C(function annotation, context; Γ_target_candidate_generation)` としてfixture上で現れることを整理する。
...
現在の入口：`40_中核音楽理論/00_中核音楽理論_計画表.md` / Module計画作成済み：`01_音高調律`〜`10_記譜綴り` / 横断レビュー：`40_中核音楽理論/11_全Module横断レビュー_破断と最小検証.md` / 作成済み検証：`10_検証/42_和声機能_同一和音とkey_context分岐_最小実験.md`〜`10_検証/48_和声機能_context差し替えによるtarget候補集合分岐_最小実験.md` / 構造抽出：`20_構造抽出/中核音楽理論_42〜45循環分解_構造抽出版.md` / `20_構造抽出/和声機能_target候補生成_46〜48構造抽出版.md`
